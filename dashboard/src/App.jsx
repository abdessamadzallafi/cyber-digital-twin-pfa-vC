...
const MAP_POSITIONS = {
  grue_G01: [18, 17], station_H01: [18, 68], portique_P01: [42, 19], camera_Q01: [43, 63],
  entrepot_E01: [68, 31], portail_N01: [71, 72], camion_C12: [84, 45], parking_P01: [85, 76]
};
...
function App() {
  ...
  if (!token) return <main className="login-page"><section className="login-panel"><div className="brand-mark">◈</div><p className="eyebrow">DIGITAL TWIN / TANGER MED</p><h1>Le port, sous contrôle.</h1><p className="login-copy">Supervisez les équipements, les opérations et la sécurité de votre terminal en temps réel.</p><form onSubmit={login}><label>Identifiant<input autoFocus value={username} onChange={e => setUsername(e.target.value)} placeholder="admin" /></label><label>Mot de passe<input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" /></label>{loginError && <p className="form-error">{loginError}</p>}<button className="primary-button" disabled={loggingIn}>{loggingIn ? 'Connexion…' : 'Accéder au poste de contrôle'} <span>→</span></button></form><p className="login-footer"><i /> Connexion sécurisée · Terminal Tanger Med</p></section><aside className="login-art"><div className="grid-glow" /><div className="art-copy"><span className="live-pill"><i /> SYSTÈME EN LIGNE</span><strong>Une vision unifiée<br />de chaque opération.</strong><p>IoT, intelligence artificielle et sécurité réunis dans une seule interface.</p></div></aside></main>;
  ...
  return <div className="shell">
    <aside className="sidebar"><div className="side-brand"><span>◈</span><div>PORT<span>OS</span><small>Digital twin</small></div></div><nav>{NAVIGATION.map(([id, name, icon]) => <button key={id} className={tab === id ? 'selected' : ''} onClick={() => setTab(id)}><b>{icon}</b>{name}{id === 'incidents' && dangerCount > 0 && <em>{dangerCount}</em>}</button>)}</nav><div className="side-bottom"><div className="operator"><span>AM</span><p>Administrateur<small>Accès sécurisé</small></p></div><button className="logout" onClick={logout}>⇥ Déconnexion</button></div></aside>
